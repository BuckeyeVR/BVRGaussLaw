...
public class VectorThingVectorSphericalHarmonics : MonoBehaviour {
		...
		if(didChange==true){
			...
			for(int i=0; i<charges.Length; i++){
				...
				for(int j = 0; j < chargeStepsInt; j++){
					...
					float theta = Mathf.Acos(transform.position.y/Mathf.Sqrt(transform.position.x*transform.position.x+transform.position.y*transform.position.y+transform.position.z*transform.position.z));
					float phi = Mathf.Atan(transform.position.z/transform.position.x);
					float scalefactor = 50.0f;
					Vector3 tmp = new Vector3(0.0f,Mathf.Sin(phi)*Mathf.Sin(theta),-Mathf.Cos(theta));
					realVector.x += scalefactor*tmp.x;
					realVector.y += scalefactor*tmp.y;
					realVector.z += scalefactor*tmp.z;
				}
			}
