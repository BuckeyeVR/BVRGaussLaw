	...
	void Update () {
		if(didChange==true){
			...
			if(float.IsNaN(realVector.magnitude)==false){
				float vectorMagnitude = realVector.magnitude / 1500;
				print (transform.GetChild (0).localScale);
				transform.GetChild(0).localScale = new Vector3(0.15f,vectorMagnitude , 0.15f);
				print (vectorMagnitude);
				//print (transform.GetChild (0).localScale);
				//print (transform.GetChild (0).localPosition);
				//transform.GetChild (0).localPosition =  new Vector3 (0, 2*(vectorMagnitude), 0);
				//print (transform.GetChild (0).localPosition);
				//print (transform.GetChild (0).position);
			}
			else{
